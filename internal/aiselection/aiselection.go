// Package aiselection resolves an org's AI connector(s) into a review.AIConfig, shared by the API layer and background workers without an import cycle.
package aiselection
...
// GetReviewAISelection resolves the org's Leader (and, if enabled, Helper) AI connector, same rules as manual/webhook reviews.
func GetReviewAISelection(ctx context.Context, db *sql.DB, orgID int64, planCode license.PlanType) (*Selection, error) {
	...
	if len(helperConnectors) == 0 {
		// Adaptive Review is on but no helper connector is configured - degrade to leader-only.
...
func selectHelperAIConfig(ctx context.Context, db *sql.DB, connectors []*aiconnectors.ConnectorRecord) (review.AIConfig, error) {
	if len(connectors) == 0 {
		// Defensive: callers should already route around this via GetReviewAISelection's check.

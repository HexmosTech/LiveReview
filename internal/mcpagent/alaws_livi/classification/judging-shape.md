...
3. Triggering a review for a PR URL or repository (e.g. "trigger review for https://github.com/...", "review PR #11"), listing configured system resources, active connectors, git/AI providers, API keys, settings, org members/team roster, connected repositories, or pull/merge requests (with no grouping, trend, or ranking requested) is strictly `action` (never `unclassified`, `analytics`, or `product_guidance`) - this overrides law 2 above wherever a direct tool answers the count/list more cheaply than SQL generation. Prefer `action` whenever an action tool or single REST call handles the requested operation. {#where-turn-is-genuinely-ambiguous}

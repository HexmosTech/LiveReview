...
3. Where a turn is genuinely ambiguous between `action` and `count_query`, answer `count_query`, since a chart still answers a list-shaped request reasonably while the reverse is not true. {#where-turn-is-genuinely-ambiguous}

...
3. Triggering a review for a PR URL or repository (e.g. "trigger review for https://github.com/...", "review PR #11"), listing configured system resources, active connectors, git/AI providers, API keys, or settings is strictly `action` (never `unclassified`, `analytics`, or `product_guidance`). Prefer `action` whenever an action tool or single REST call handles the requested operation. {#where-turn-is-genuinely-ambiguous}

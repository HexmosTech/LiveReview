...
3. Answer `action` where the user wants something done — a review triggered, a learning created, a connector added — or wants to see the organization's own configuration/settings (connectors, integrations, quota, learnings) rather than data generated by using the product (reviews, feedback, lines of code). A single named record, or a short list of configured items, that one tool call answers directly is also `action`. {#answer-action-where-the-user}

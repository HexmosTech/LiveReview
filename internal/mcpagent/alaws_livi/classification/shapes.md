...
3. Answer `action` for any request to trigger or run a code review (e.g. "trigger review for https://github.com/...", "review PR #11", "run review for repo X"), execute an operation (creating/updating learnings, settings), or view/list current system setup or inventory (connectors, providers, keys, quota, review details, org members/team roster, repositories, pull/merge requests). Any query asking to trigger a review for a PR URL or repo, or to list/inspect setup state, org membership, repositories, or pull/merge requests, MUST be classified as `action` (never `unclassified`, `analytics`, or `product_guidance`). {#answer-action-where-the-user}

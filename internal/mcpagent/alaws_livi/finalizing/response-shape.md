...
2. Set `response_type` to either `chart` or `csv`, and must include `data_sql`, the query that produces the answer's rows.

3. Include `title`, `description`, `query`, `time_range` and `granularity` on every response.

4. Describe a chart with either `mark` and `encoding` together, or `layer` for several marks in one panel, or `facet` and `spec` for the same mark repeated once per category — and must never combine `layer` with `facet`, because layers overlay in a single panel while facets split across panels.

...
8. Where a baseline or target line's value is already a column in the data being plotted — a period average, a fixed threshold repeated on every row — encode it with an `"aggregate"` (`mean`, `min`, `max`) directly against that same dataset, and never introduce a second `data` block holding one literal value for the layer to reference. A second data block requires that value to be filled in by hand at the moment the spec is written, and a placeholder left unfilled or copied literally renders as nothing — the line silently fails to draw instead of erroring. {#where-baseline-or-target-line}

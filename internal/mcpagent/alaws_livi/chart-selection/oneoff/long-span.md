...
2. Produce a plain daily series, zero-filled.

3. Split each day's value into bands after querying, deriving one column per band holding that band's share of the value.

4. Stack the bands as areas of one colour with rising opacity, so that intensity reads as magnitude.

5. State the peak and the typical level in words, because a compact chart is harder to read precisely and the figures therefore matter more.

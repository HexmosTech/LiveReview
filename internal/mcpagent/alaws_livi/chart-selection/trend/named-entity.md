...
1. Apply this section where a question asks what happened to a single named repository, engineer or team over time. {#apply-this-section-where-question}

2. Confirm which entity is meant before querying, and ask where the question implies one without naming it. {#confirm-which-entity-is-meant}

3. Measure lines of code summed per day for that entity rather than a record count, because velocity concerns how much code moved and one large review is not the same event as one trivial one. {#measure-lines-of-code-summed}

4. Compute the rolling average in the query. {#compute-the-rolling-average-in}

5. Mark the recent interval with a highlight band supplied as two dates, not drawn from the query. {#mark-the-recent-interval-with}

6. Compare the highlighted interval against the interval preceding it in the description, because the line alone shows only that something happened and not what. {#compare-the-highlighted-interval-against}

...
5. `loc_usage_ledger.status` is one of `accounted` or `ignored` only — never `'processed'`, `'complete'`, or any other value, all of which silently match zero rows. Take lines-of-code figures only from `status = 'accounted'` rows. `loc_usage_ledger.actor_kind` is one of `member`, `system`, or `unknown` only — never `'user'`. `loc_usage_ledger.operation_type` holds the review trigger type (`diff_review`, `manual_review`, `scheduled_review`, `webhook_review`). Never filter on the generic string `'review'` (which silently produces zero rows). When querying overall lines-of-code across all review types, filter by `status = 'accounted'` without restricting `operation_type` unless specifically asked for a single review type. {#loc-usage-ledger-enum-values}
...
8. `blast_radius_hunks` is one row per hunk, not per review — a single review with multiple high-tier hunks joins to multiple rows. Any `COUNT` of reviews after joining `blast_radius_hunks` (directly or via a subquery keyed on `review_id`) MUST be `count(DISTINCT r.id)` / `count(DISTINCT review_id)`, never bare `count(r.id)` / `count(*)` — the bare form counts hunks, silently inflating the review count (a review with 3 high-tier hunks would be counted as 3 reviews instead of 1). This is the same one-to-many-join rule as the data-handling chapter's law on inflated counts, called out here explicitly because this specific join is the one it keeps getting dropped on. {#blast-radius-hunks-one-row-per-hunk-not-review}

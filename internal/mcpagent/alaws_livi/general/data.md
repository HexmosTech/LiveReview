...
4. Take lines-of-code figures only from settled ledger rows (`loc_usage_ledger.status = 'accounted'`). Counting provisional rows lets unaccounted numbers leak into history. `loc_usage_ledger.operation_type` holds the review trigger type (`diff_review`, `manual_review`, `scheduled_review`, `webhook_review`). Never filter on the generic string `'review'` (which silently produces zero rows). When querying overall lines-of-code across all review types, filter by `status = 'accounted'` without restricting `operation_type` unless specifically asked for a single review type. {#take-lines-of-code-figures}

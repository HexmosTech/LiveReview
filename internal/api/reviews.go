...
func (rm *ReviewManager) AddAIComment(reviewID int64, commentType string, content map[string]interface{}, filePath *string, lineNumber *int) (*AIComment, error) {
	...
	query := `
		INSERT INTO ai_comments (review_id, comment_type, content, file_path, line_number)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, created_at
	`

	var comment AIComment
	err = rm.db.QueryRow(query, reviewID, commentType, contentJSON, filePath, lineNumber).Scan(&comment.ID, &comment.CreatedAt)
	if err != nil {
...
func (rm *ReviewManager) GetReviewComments(reviewID int64) ([]AIComment, error) {
	query := `
		SELECT id, review_id, comment_type, content, file_path, line_number, created_at
		FROM ai_comments
		WHERE review_id = $1
...
func TrackAICommentFromURL(db *sql.DB, prMrURL, commentType string, content map[string]interface{}, filePath *string, lineNumber *int) error {
	...
	_, err = reviewManager.AddAIComment(reviewID, commentType, content, filePath, lineNumber)
	if err != nil {

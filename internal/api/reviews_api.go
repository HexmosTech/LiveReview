...
type reviewAISelection = aiselection.Selection
...
// getReviewAISelectionFromDatabase resolves the org's configured AI connector(s). The actual
// logic lives in internal/aiselection so background workers (e.g. scheduled reviews) can
// reuse it without importing the API package.
func (s *Server) getReviewAISelectionFromDatabase(ctx context.Context, orgID int64, planCode license.PlanType) (*reviewAISelection, error) {
	return aiselection.GetReviewAISelection(ctx, s.db, orgID, planCode)
}

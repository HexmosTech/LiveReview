...
func (s *Server) ValidateAIConnectorKey(c echo.Context) error {
	...
	if req.Provider == "bedrock" && strings.TrimSpace(req.AWSAccessKeyID) == "" {
		return c.JSON(http.StatusBadRequest, AIConnectorKeyValidationResponse{
			Valid:   false,
	...
	valid, err := aiconnectors.ValidateAPIKey(
		...
		strings.TrimSpace(req.AWSAccessKeyID),
		strings.TrimSpace(req.AWSRegion),
	)
...
func (s *Server) CreateAIConnector(c echo.Context) error {
	...
	connector := &aiconnectors.ConnectorRecord{
		...
		AWSAccessKeyID: sql.NullString{String: strings.TrimSpace(req.AWSAccessKeyID), Valid: strings.TrimSpace(req.AWSAccessKeyID) != ""},
		AWSRegion:      sql.NullString{String: strings.TrimSpace(req.AWSRegion), Valid: strings.TrimSpace(req.AWSRegion) != ""},
...
func (s *Server) UpdateAIConnector(c echo.Context) error {
	...
	existingConnector.AWSAccessKeyID = sql.NullString{String: strings.TrimSpace(req.AWSAccessKeyID), Valid: strings.TrimSpace(req.AWSAccessKeyID) != ""}
	existingConnector.AWSRegion = sql.NullString{String: strings.TrimSpace(req.AWSRegion), Valid: strings.TrimSpace(req.AWSRegion) != ""}
...
// FetchBedrockModelsRequest represents the request for fetching Bedrock foundation models
type FetchBedrockModelsRequest struct {
	AccessKeyID     string `json:"access_key_id,omitempty"`
	SecretAccessKey string `json:"secret_access_key,omitempty"`
	Region          string `json:"region"`
}

// FetchBedrockModels handles requests to list available foundation models for a Bedrock
// connector, using the credentials the admin just entered in the form (mirrors FetchOllamaModels).
func (s *Server) FetchBedrockModels(c echo.Context) error {
	orgIDVal := c.Get("org_id")
	if _, ok := orgIDVal.(int64); !ok {
		return c.JSON(http.StatusBadRequest, map[string]string{
			"error": "Organization context required",
	...
	var req FetchBedrockModelsRequest
	if err := c.Bind(&req); err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{
			"error": "Invalid request format",
		})
	...
	log.Info().
		...
		Msg("Fetching foundation models from Bedrock")

	ctx, cancel := context.WithTimeout(c.Request().Context(), 30*time.Second)
	defer cancel()

	models, err := aiconnectors.FetchBedrockModels(ctx, strings.TrimSpace(req.AccessKeyID), req.SecretAccessKey, strings.TrimSpace(req.Region))
	if err != nil {
		log.Error().Err(err).
			Str("region", req.Region).
			Msg("Failed to fetch models from Bedrock")
		return c.JSON(http.StatusBadRequest, map[string]string{
			"error": fmt.Sprintf("Failed to fetch models: %v", err),
		})
	}
	...
	log.Info().
		...
		Msg("Successfully fetched Bedrock foundation models")

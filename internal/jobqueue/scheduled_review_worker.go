...
// ScheduledReviewWorker diffs a repo's default branch since the last checkpoint and runs it through the standard AI review engine; unlike other review flows it never persists the diff itself, only comments and the SHA checkpoint.
...
func (w *ScheduledReviewWorker) Work(ctx context.Context, job *river.Job[ScheduledReviewJobArgs]) error {
	...
	err = w.db.QueryRowContext(ctx, `SELECT provider, provider_url, pat_token FROM integration_tokens WHERE id = $1`, cfg.ConnectorID).
	...
	if baseSHA == "" {
		lookbackInterval, intervalErr := scheduledreviewstore.ApproxInterval(cfg.CronExpression, time.Now())
		if intervalErr != nil {
			log.Printf("[WARN] ScheduledReviewWorker: config %d could not estimate lookback interval, defaulting to 24h: %v", cfg.ID, intervalErr)
	...
	// Coarse safety gate: don't let the scheduler keep spending AI budget silently once the org is blocked.

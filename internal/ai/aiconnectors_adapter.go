...
func (a *AIConnectorsAdapter) ReviewCode(ctx context.Context, diffs []*models.CodeDiff) (*models.ReviewResult, error) {
	...
	connectors, err := a.storage.GetConnectorsByProvider(ctx, a.orgID, a.provider)
	if err != nil {
		return nil, fmt.Errorf("failed to get connectors: %w", err)
	}

	if len(connectors) == 0 {
		return nil, fmt.Errorf("no connectors found for provider %s in org %d", a.provider, a.orgID)
	}
	...
	prompt := base + "\n\n" + prompts.BuildCodeChangesSectionWithContext(ctx, diffs)

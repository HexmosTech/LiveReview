...
# The chatbot's RAG corpus (internal/docindex/docs/{lrc_wiki,lr_wiki,hexmos_docs},
# go:embed target) is committed to git and arrived via COPY . . above,
# already matching the commits pinned in scripts/docindex/docs_sources.env
# (also committed) - this is a self-heal/freshness check, expected to be a
# no-op (no network) unless the committed content and the pin have drifted.
# See docs/docs-sources-pinning-plan.md.

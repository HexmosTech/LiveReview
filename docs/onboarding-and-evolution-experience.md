...
4. **Inflexible Architecture**: Moving between deployment phases (localhost → server → domain) requires significant reconfiguration4. **Inflexible Architecture**: Moving between deployment phases (localhost → server → domain) requires significant reconfiguration
...
- **Pattern**: `localhost:8081` → `localhost:8888`- **Pattern**: `localhost:8081` → `localhost:8888`
...
- **Pattern**: `domain.com` or `ip-address` → Proxy → Backend- **Pattern**: `domain.com` or `ip-address` → Proxy → Backend

- **API Calls**: `{current-origin}/api/*` → Backend- **API Calls**: `{current-origin}/api/*` → Backend

- **Webhooks**: `{current-origin}/api/v1/gitlab-hook` → Backend- **Webhooks**: `{current-origin}/api/v1/gitlab-hook` → Backend
...
setup_configuration() {setup_configuration() {
    ...
    echo "🔧 LiveReview Configuration"    echo "🔧 LiveReview Configuration"
...
start_servers() {              <Server className="h-5 w-5 text-gray-400 mr-3" />
    ...
    echo "🚀 Starting servers on ports ${frontend_port}/${backend_port}"            </div>
...
1. **Set up reverse proxy** (nginx/caddy/apache) to route:          </div>
   ...
   - `/api/*` → `localhost:8888` (backend)          {/* Webhook Status */}
...
start_servers() {
    ...
    echo "🚀 Starting servers on ports ${frontend_port}/${backend_port}"
...
4. **Inflexible Architecture**: Moving between deployment phases (localhost → server → domain) requires significant reconfiguration
...
- **Pattern**: `localhost:8081` → `localhost:8888`
...
- **Pattern**: `domain.com` or `ip-address` → Proxy → Backend
- **API Calls**: `{current-origin}/api/*` → Backend
- **Webhooks**: `{current-origin}/api/v1/gitlab-hook` → Backend
...
```
Frontend (localhost:8081) → Backend (localhost:8888)
...
```
Frontend (server-ip:8081) → Backend (server-ip:8888)
Git Webhooks → Backend (server-ip:8888/api/v1/gitlab-hook)
...
```
Browser → Proxy → Frontend (127.0.0.1:8081)
API calls → Proxy (/api/*) → Backend (127.0.0.1:8888)  
Git Webhooks → Proxy → Backend (127.0.0.1:8888/api/v1/gitlab-hook)
...
1. **lrops.sh Initial Setup** → Creates `.env` file with basic configuration
2. **Docker Compose** → Reads `.env` file and passes to containers
3. **docker-entrypoint.sh** → Uses environment variables to start services
4. **Go Backend (cmd/api.go, cmd/ui.go)** → Reads ports from CLI flags/env vars
5. **Frontend Runtime** → Receives API URL via `window.LIVEREVIEW_CONFIG` injection
...
| `LIVEREVIEW_API_URL` | auto | Frontend→Backend API URL | lrops.sh, auto-resolved | docker-entrypoint.sh, UI injection |

...
const UserForm: React.FC = () => {
    ...
    try {
      ...
      setBulkRows(
        results.map((row, index) => {
          ...
          return {
            ...row,
            role: allowedRoles.includes(normalizedRole) ? normalizedRole : 'member',
            id: `${index}-${row.email}`,
      ...
      toast.error('Failed to verify users against your organization. Please try again.');
      setBulkFile(null);
    } finally {
  ...
  const updateBulkRow = (id: string, patch: Partial<BulkRow>) => {
    setBulkRows((rows) => (rows ? rows.map((row) => (row.id === id ? { ...row, ...patch } : row)) : rows));
  };

  const removeBulkRow = (id: string) => {
    setBulkRows((rows) => (rows ? rows.filter((row) => row.id !== id) : rows));
  };
      ...
      setBulkRows((rows) =>
        rows
          ? rows.map((row, index) => {
              ...
              return result ? { ...row, submit_status: result.status, submit_message: result.message } : row;
            })
          : rows
      );
            ...
            {!isEditMode && activeTab === 'bulk' ? (
              bulkVerifying ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <Spinner size="lg" />
                  <p className="mt-4 text-sm text-slate-300">Verifying users against your organization…</p>
                </div>
              ) : bulkRows ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-slate-300">
                      {bulkRows.length} user{bulkRows.length !== 1 ? 's' : ''} ready to review
                    </p>
                    <button
                      type="button"
                  ...
                  {bulkRows.length > 0 ? (
                    <div className="overflow-x-auto border border-slate-700 rounded-lg">
                      <table className="w-full text-sm text-left" style={{ minWidth: 720 }}>
                        <thead className="bg-slate-900/60 text-slate-400 uppercase text-xs">
                          <tr>
                            <th className="px-4 py-2.5 whitespace-nowrap">Email</th>
                            <th className="px-4 py-2.5 whitespace-nowrap">First Name</th>
                            <th className="px-4 py-2.5 whitespace-nowrap">Last Name</th>
                        ...
                        <tbody className="divide-y divide-slate-700">
                          {bulkRows.map((row) => {
                            const hasGlobalAccount = row.exists_globally && !row.exists;
                            return (
                            <tr key={row.id} className="align-top">
                              <td className="px-4 py-3 text-slate-200 whitespace-nowrap">
                                {row.email}
                                {row.old_email && (
                                  <p className="text-xs text-amber-400 mt-1">was: {row.old_email}</p>
                                )}
                              </td>
                              <td className="px-4 py-3 min-w-[140px]">
                                <input
                                  value={row.first_name}
                                  onChange={(e) => updateBulkRow(row.id, { first_name: e.target.value })}
                                  disabled={bulkSubmitting || hasGlobalAccount}
                                ...
                                />
                                {row.old_first_name && (
                                  <p className="text-xs text-amber-400 mt-1">was: {row.old_first_name}</p>
                                )}
                              </td>
                              <td className="px-4 py-3 min-w-[140px]">
                                <input
                                  value={row.last_name}
                                  onChange={(e) => updateBulkRow(row.id, { last_name: e.target.value })}
                                  disabled={bulkSubmitting || hasGlobalAccount}
                                  className="w-full bg-slate-900 border border-slate-600 rounded px-2.5 py-1.5 text-white text-sm outline-none focus:border-blue-500 disabled:opacity-50"
                                />
                                {row.old_last_name && (
                                  <p className="text-xs text-amber-400 mt-1">was: {row.old_last_name}</p>
                                )}
                              </td>
                              <td className="px-4 py-3 min-w-[140px]">
                                <div className="relative">
                                  <select
                                    value={row.role}
                                    onChange={(e) => updateBulkRow(row.id, { role: e.target.value })}
                                    disabled={bulkSubmitting}
                                ...
                                {row.old_role && (
                                  <p className="text-xs text-amber-400 mt-1">was: {row.old_role}</p>
                                )}
                              </td>
                    ...
                    <p className="text-sm text-slate-400 text-center py-6 border border-dashed border-slate-700 rounded-lg">
                      All rows removed. Upload a different file to continue.
                    </p>
                  )}
                ...
                <div className="space-y-5">
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-lg py-14 px-6 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${
                      isDragging ? 'border-blue-500 bg-slate-700/40' : 'border-slate-600 hover:border-slate-500 bg-slate-900/30'
                    }`}
                  >
                    <span className="text-slate-200">
                      <CloudUploadIcon />
                    </span>
                    <p className="mt-3 text-base text-slate-200">Drag &amp; drop or browse</p>
                    <input

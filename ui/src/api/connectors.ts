...
export const getConnector = async (connectorId: string): Promise<ConnectorResponse> => {
  try {
    const response = await apiClient.get<ConnectorResponse>(`/connectors/${connectorId}`);
    return response;
  } catch (error) {
...
export const getConnectors = async (): Promise<ConnectorResponse[]> => {
  try {
    return await apiClient.get<ConnectorResponse[]>('/connectors');
  } catch (error) {
...
export const getRepositoryAccess = async (connectorId: string, refresh?: boolean): Promise<RepositoryAccess> => {
  try {
    let url = `/connectors/${connectorId}/repository-access`;
    if (refresh) {
      url += '?refresh=true';
    }
...
export const enableManualTriggerForAllProjects = async (connectorId: string): Promise<any> => {
  try {
    const response = await apiClient.post(`/connectors/${connectorId}/enable-manual-trigger`, {});
    return response;
  } catch (error) {
...
export const disableManualTriggerForAllProjects = async (connectorId: string): Promise<any> => {
  try {
    const response = await apiClient.post(`/connectors/${connectorId}/disable-manual-trigger`, {});
    return response;
  } catch (error) {

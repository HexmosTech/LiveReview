...
Usage:
  python3 scripts/release_gh.py --repo HexmosTech/LiveReview [--version vX.Y.Z]
"""
...
def infer_from_latest_tag() -> Optional[str]:
    tags = run(["git", "tag", "--list", "v*"], check=False)
    if not tags:
        return None
    return semver_max([t.strip() for t in tags.splitlines() if t.strip()])
...
def infer_version(explicit: Optional[str]) -> str:
    ...
    from_latest = infer_from_latest_tag()
    if from_latest:
        return validate_version(from_latest)
